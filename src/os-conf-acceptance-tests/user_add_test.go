...
var _ = Describe("UserAdd", func() {
	It("allows users to add users to the VM", func() {
		...
		By("adding a password for the password user", func() {
			...
			Eventually(session, 30*time.Second).Should(gbytes.Say("test-user-password:\\$6\\$kMBogqsbx\\$70Y2m/mwYR8vKZqR9RD2UUPoWz8mJoBiH8IAbvH2v6LCjxJgB3kDtwR8QttqtI/WSqCsFy4qXZaKPM64sZMwK\\.:17702:1:99999:7:::"))
			Eventually(session, 30*time.Second).Should(gbytes.Say("test-user-key::17702:1:99999:7:::"))
			Eventually(session, 30*time.Second).Should(gexec.Exit(0))
		})
